...
if((attack && attackH) || (attack && attackH)){
      if (vAniID == openborconstant("ANI_follow17")){ //In valid animations?
	performattack(vEnt, openborconstant("ANI_follow18"));
	}
...
if((jump && jumpH) || (jump && jumpH)){
	...
	performattack(vEnt, openborconstant("ANI_follow17"));
