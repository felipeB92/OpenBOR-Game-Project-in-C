...
int getFPS() {
    ...
    if ( ov("in_level") && !ov("game_paused") ) {
        ...
        if ( (getlocalvar("fps_time") >= 0 && time-getlocalvar("fps_time") >= game_speed) || (getlocalvar("fps_time") < 0 && abs(time-getlocalvar("fps_time")) >= game_speed) ) {
            ...
            average = abs(trunc((average+getlocalvar("fps"))/2.0));
...
void flashing3(void self, int colour1, int colour2, int reset_flag) {
    int time = openborvariant("elapsed_time");
    int flash_frames = 8;
    float time_range = trunc(flash_frames/3);
    int no_colour = rgbcolor(0x00,0x00,0x00);
    //int colour1 = rgbcolor(0xFF,0xFF,0x00);
    //int colour2 = rgbcolor(0xFF,0x00,0x00);
...
int autokill_key(int p, char key, char attack_type, int back_flag) {
    if ( ov("in_level") ) {
        if ( glvp("type") != 2 ) {
...
int is_incam(void ent, float threshold) {
    if ( getentityproperty(ent,"exists") && openborvariant("in_level") ) {
        ...
        if ( x >= scr_posx+threshold && x <= scr_posx+hres-threshold && z-base >= scr_posy+threshold && z-base <= scr_posy+vres-threshold ) {
            if ( x >= scrollminx+threshold && x <= scrollmaxx-threshold && z >= minz+threshold && z <= maxz-threshold ) {
                return 1;
            }
        }
    }
...
void playanigif(void path) {
  ...
  int time = openborvariant("elapsed_time");
	...
	if(!gif) {
        ...
        //finished?
		if ( decodeanigif(gif) || (getlocalvar("skipable_gif") >= 1 && anykey() ) ) {
		    destroyanigif();
            gif = reopengif(path);
        }
	}
